...
def glue_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if output_mode == "classification":
            ...
            label = [int(item) for item in example.label]
        elif output_mode == 'detection':
            label = np.array(eval(example.label)).reshape(-1, 2)
        else:
...
class DnaSegProcessor(DataProcessor):
    """Processor for the DNA promoter data"""
...
class DnaDetProcessor(DataProcessor):
    """Processor for the DNA promoter data"""
...
glue_processors = {
    ...
    "dnaseg": DnaSegProcessor,
    "dnadet": DnaDetProcessor
}
