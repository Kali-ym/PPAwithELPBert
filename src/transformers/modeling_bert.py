...
from .detection.rpn_network import RegionProposalNetwork
from .segmentation.decoder import UNet, DecoderLinear
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # self.conv1 = nn.Conv1d(config.hidden_size, config.hidden_size, kernel_size=(13,), stride=(1,), padding=6)
        # self.relu = nn.ReLU()
        # self.conv2 = nn.Conv1d(config.hidden_size, config.hidden_size, kernel_size=(13,), stride=(1,), padding=6)
    ...
    Examples::
        ...
        # TODO 加两层CNN
        # embedding_output = self.relu(self.conv2(self.relu(self.conv1(embedding_output.transpose(-1, -2))))).transpose(-1, -2)
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    Returns:
        ...
        if masked_lm_labels is not None:
            ...
            masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), masked_lm_labels.view(-1))
            pred = torch.argmax(prediction_scores.view(-1, self.config.vocab_size), dim=1)
            total = (masked_lm_labels.view(-1) != -100).sum()
            acc = float((pred == masked_lm_labels.view(-1)).cpu().numpy().astype(int).sum()) / float(
                total.cpu().numpy().astype(int))
            outputs = (masked_lm_loss, acc) + outputs
...
class BertForSequenceSegmentation(BertPreTrainedModel):
    def __init__(self, config, linear=True):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = DecoderLinear(config.num_labels, config.hidden_size) if linear else \
            UNet(config.num_labels, config.hidden_size)
    ...
    Examples::
        ...
        '''
        output = outputs[0]
        real_len = [len(l) - self.config.kmer + 1 for l in labels]
        max_len = max(real_len)
        inputs = []
        for i, le in enumerate(real_len):
            seq = output[i, 1:le + 1, :]
            zero_padding = torch.zeros([max_len - seq.shape[-2], self.config.hidden_size], device=output.device,
                                       dtype=output.dtype)
            real_seq = torch.cat((seq, zero_padding))
            inputs.append(torch.unsqueeze(real_seq, dim=0))
        inputs = torch.cat(inputs, dim=0)
        logits = self.classifier(self.dropout(inputs.transpose(-1, -2)))
        logits = logits['logits']
        merge_label = torch.zeros([1], device=output.device, dtype=output.dtype)
        merge_logits = torch.zeros([1, self.config.num_labels], device=output.device, dtype=output.dtype)
        real_len = [len(l) for l in labels]
        for i, l in enumerate(real_len):
            seq = torch.unsqueeze(logits[i], dim=0)
            merge_logits = torch.cat((merge_logits, interpolate(seq, [l], mode='linear').squeeze(0).transpose(-1, -2)))
            label = torch.as_tensor(labels[i], device=output.device, dtype=output.dtype)
            merge_label = torch.cat((merge_label, label))
        '''
        output = outputs[0]
        real_len = [len(le) for le in labels]
        max_len = max(real_len)
        inputs = []
        merge_seq = torch.zeros([1, self.config.hidden_size], device=output.device, dtype=output.dtype)
        merge_label = torch.zeros([1], device=output.device, dtype=output.dtype)
        merge_logits = torch.zeros([1, self.config.num_labels], device=output.device, dtype=output.dtype)
        for i, length in enumerate(real_len):
            real_seq = torch.zeros([length, self.config.hidden_size], device=output.device, dtype=output.dtype)
            count = torch.zeros([length], device=output.device, dtype=output.dtype)
            seq = output[i, 1:length - self.config.kmer + 2, :]
            for j in range(0, length - self.config.kmer + 1):
                for k in range(self.config.kmer):
                    count[j + k] += 1
                    real_seq[j + k] += seq[j]
            real_seq = real_seq / count.reshape(-1, 1)
            zero_padding = torch.zeros([max_len - length, self.config.hidden_size], device=output.device,
                                       dtype=output.dtype)
            real_seq = torch.cat((real_seq, zero_padding))
            inputs.append(torch.unsqueeze(real_seq, dim=0))
            # merge_seq = torch.cat((merge_seq, real_seq))
        ...
        for i, (length) in enumerate(real_len):
            label = torch.as_tensor(labels[i], device=output.device, dtype=output.dtype)
            merge_label = torch.cat((merge_label, label))
            merge_seq = torch.cat((merge_seq, inputs[i, :length]))
            merge_logits = torch.cat((merge_logits, logits[i, :length]))

        output = merge_seq[1:]
        labels = merge_label[1:]
        logits = merge_logits[1:]

        outputs = (logits, labels, real_len) + outputs[2:]  # add hidden states and attention if they are here
...
class BertForSequenceDetection(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.bert = BertModel(config)
        self.detection = RegionProposalNetwork(config.hidden_size, num_anchors=8)
    ...
    Examples::
        ...
        output = outputs[0]
        real_len = [(int(torch.sum(le)) + self.config.kmer - 3) for le in attention_mask]
        max_len = max(real_len)
        labels = [l.to(device=output.device, dtype=output.dtype) for l in labels]
        inputs = []
        for i, (length) in enumerate(real_len):
            real_seq = torch.zeros([length, self.config.hidden_size], device=output.device, dtype=output.dtype)
            count = torch.zeros([length], device=output.device, dtype=output.dtype)
            ...
            inputs.append(torch.unsqueeze(real_seq, dim=0))
        inputs = torch.cat(inputs, dim=0)
        loss, boxes = self.detection(inputs, labels, real_len)

        outputs = (loss, boxes) + outputs[2:]  # add hidden states and attention if they are here
