...
from transformers import (
    ...
    BertForSequenceSegmentation,
    BertForSequenceDetection,
...
MODEL_CLASSES = {
    ...
    "dnalong": (BertConfig, BertForSequenceSegmentation, DNATokenizer),
...
def train(args, train_dataset, model, tokenizer):
    ...
    # train_sampler = BatchSampler(train_dataset, batch_size=args.train_batch_size, drop_last=False) if args.local_rank == -1 else DistributedSampler(train_dataset)
...
def heatmaps(preds, out_label_ids, real_len):
    def sets(p, o):
        if p == 0 and o == 0:
            return 0
        elif p == 0 and o == 1:
    ...
    step = 0
    output = [sets(p, o) for p, o in zip(preds, out_label_ids)]
    outputs = []
    for l in real_len:
        outputs.append(output[step:step + l])
        step = step + l
    for i in range(0, len(outputs), 10):
        ...
        for t in range(0, len(temp)):
            padding = maxl - ll[t]
            temp[t] = temp[t] + [-1] * padding
        data = np.array(temp)
        data = pd.DataFrame(data)

        tick_ = np.arange(-1, 4, 1).astype(float)

        # dict_ = {'orientation': 'vertical', "label": "color  \
        # scale", "drawedges": True, "ticklocation": "top", "extend": "min", "filled": True, "alpha": 0.8, "cmap": "cmap", "ticks": tick_}

        cmap = sns.heatmap(data, center=0)
...
def printto(preds, real_len):
    step = 0
    path = "./sample_data/"
    file = open(path + "validation_test.data", "w")
    for l in real_len:
        padding = 1024 - l
        pp = list(preds[step:step + l]) + [-1.0] * padding
        # gg = list(out_label_ids[step:step + l]) + [-1.0] * padding
        p = str(pp)
        # g = str(gg)
        p = p[1:-1].replace(',', '')
        # g = g[1:-1].replace(',', '')
        o = p
        file.write(o.replace('\n', '') + '\n')
        step = step + l
    file.close()
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if preds is None:
                ...
                out_label_ids = outputs[2].detach().cpu().numpy()
                real_len = outputs[3]
            else:
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                out_label_ids = np.append(out_label_ids, outputs[2].detach().cpu().numpy(), axis=0)
                real_len = real_len + outputs[3]

        eval_loss = eval_loss / nb_eval_steps
        results.update({"loss": eval_loss})
        if args.output_mode == "classification" or args.output_mode == "segmentation":
            ...
            printto(probs, real_len)
            preds = np.argmax(preds, axis=1)
            # heatmaps(preds, out_label_ids, real_len)
        elif args.output_mode == "regression":
            preds = np.squeeze(preds)
        if args.do_ensemble_pred:
            result = compute_metrics(eval_task, preds, out_label_ids, probs[:, 1])
        else:
            # skip = sum([o for o in real_len if o > 400])
            # kk = np.random.randint(0, 2, (110112,), preds.dtype)
            result = compute_metrics(eval_task, preds, out_label_ids, probs)
            for p, o in zip(preds, out_label_ids.astype(int)):
                if p == 1 and o == 1:
                    tp += 1
                elif p == 1 and o == 0:
                    fp += 1
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    args.eval_batch_size = args.per_gpu_eval_batch_size * max(1, args.n_gpu)
    batch_size = args.eval_batch_size if evaluate else args.train_batch_size
    ...
    elif output_mode == "segmentation":
        all_labels = [f.label for f in features]
    elif output_mode == "detection":
        all_labels = [torch.tensor(f.label, dtype=torch.float) for f in features]

    all_input_ids = [all_input_ids[i:i + batch_size] for i in range(0, len(all_input_ids), batch_size)]
    all_attention_mask = [all_attention_mask[i:i + batch_size] for i in range(0, len(all_attention_mask), batch_size)]
    all_token_type_ids = [all_token_type_ids[i:i + batch_size] for i in range(0, len(all_token_type_ids), batch_size)]
    all_labels = [all_labels[i:i + batch_size] for i in range(0, len(all_labels), batch_size)]
    dataset = DNADataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels)
    return dataset
