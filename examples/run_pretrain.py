...
class TextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
        ...
        self.examples = []
        logger.info("Loading features file %s", file_path)
    ...
    def __getitem__(self, item):
        x = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(self.examples[item]))
        return torch.tensor(self.tokenizer.build_inputs_with_special_tokens(x), dtype=torch.long)
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    eval_accu = eval_accu / nb_eval_steps
    perplexity = torch.exp(torch.tensor(eval_loss))

    result = {"perplexity": perplexity, "accu": eval_accu}
